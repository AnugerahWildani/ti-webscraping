...
PYTHONSCRIPTPATH = r'C:\Users\IssueConAdmin\Python37\Scripts'
# The directory name of django project
# Note: it is the directory at the same level of manage.py
# not the parent directory
PROJECTDIR = 'djangoweb'

logging.basicConfig(
    filename = os.path.join(INSTDIR, 'celery_worker_service.log'),
    level = logging.DEBUG, 
...
class CeleryService(win32serviceutil.ServiceFramework):
    ...
    _svc_display_name_ = "Celery Worker Distributed Task Queue Service"
    ...
    def SvcDoRun(self):
        ...
        command = '"{celery_path}" -A {proj_dir} worker -f "{log_path}" -l info'.format(
            ...
            proj_dir=PROJECTDIR,
            log_path=os.path.join(INSTDIR,'celery_worker.log'))
